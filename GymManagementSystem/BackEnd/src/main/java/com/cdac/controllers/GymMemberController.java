	...
	private ActivityService activityService;
	   ...
	   //get Gym Member  by memberId
		...
		//delete GymMembers
		   ...
		   @PostMapping("/gymmembers/{memberId}/assignDietPlan")
		   public ResponseEntity<GymMember> assignDietPlan (@PathVariable("memberId") int id, @RequestBody DietPlan plan){
			   int dietplanId = plan.getplanId();
			   DietPlan optionalDietplan = dietPlanService.findById(dietplanId);
			   GymMember member = gymMemberService.findById(id);
			   member.setDietPlan(optionalDietplan);
			   GymMember gm = gymMemberService.save(member);
			   return ResponseEntity.ok(gm);
			   
		   }
		   
		   @PostMapping("/gymmembers/{memberId}/assignWorkoutPlan")
		   public ResponseEntity<GymMember> assignWorkoutPlan (@PathVariable("memberId") int id, @RequestBody WorkoutPlan plan){
			   int workoutid = plan.getWorkoutid();
			   WorkoutPlan optionalWorkoutPlan = workoutPlanService.findById(workoutid);
			   GymMember member = gymMemberService.findById(id);
			   member.setWorkoutPlan(optionalWorkoutPlan);
			   GymMember gm = gymMemberService.save(member);
			   return ResponseEntity.ok(gm);
